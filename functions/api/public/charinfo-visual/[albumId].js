...
export async function onRequestGet({ request, env, params }) {
    ...
    if (!albumId) return json({ error: 'Visual profile not found' }, 404);
    ...
    if (!album) return json({ error: 'Visual profile not found' }, 404);
...
export function albumIdFromRequest(requestUrl) {
    const prefix = '/api/public/charinfo-visual/';
