...
export async function onRequestGet({ request, env }) {
    ...
    const rows = await env.img_d1.prepare('SELECT id, file_name, file_type, file_size_bytes, timestamp, width, height, visibility, discover_eligible, moderation_status, metadata FROM files WHERE owner_id = ? ORDER BY timestamp DESC LIMIT 200').bind(identity.id).all();
