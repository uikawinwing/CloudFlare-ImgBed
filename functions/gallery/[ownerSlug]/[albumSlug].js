export async function onRequestGet({ request, env }) {
    const slugs = gallerySlugsFromRequest(request.url, '/gallery/');
    if (!slugs) return new Response('Gallery not found', { status: 404 });
    const { ownerSlug, albumSlug } = slugs;
    const owner = escapeAttribute(ownerSlug);
    const album = escapeAttribute(albumSlug);
    const record = env.img_d1?.prepare ? await env.img_d1.prepare("SELECT a.name, a.description, u.username FROM albums a JOIN users u ON u.discord_id = a.owner_id WHERE u.public_handle = ? AND a.slug = ? AND a.visibility = 'public'").bind(ownerSlug, albumSlug).first() : null;
    const name = escapeAttribute(record?.name || albumSlug || '公开图库');
    const creator = escapeAttribute(record?.username || ownerSlug || '');
    const description = escapeAttribute(record?.description || '');
    return new Response(`<!doctype html><html lang="zh-CN"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1,viewport-fit=cover"><meta name="theme-color" content="#f6f4ef"><meta name="description" content="${description || name}"><link rel="icon" href="/static/media/logo.png"><script>try{document.documentElement.dataset.theme=localStorage.getItem('imgbed-theme')||((matchMedia('(prefers-color-scheme: dark)').matches)?'dark':'light')}catch{}</script><link rel="stylesheet" href="/theme.css"><link rel="stylesheet" href="/gallery-app/gallery.css"><title>${name} · CloudFlare ImgBed</title></head><body data-owner="${owner}" data-album="${album}" data-name="${name}" data-creator="${creator}" data-description="${description}"><main id="gallery-app"></main><noscript>需要启用 JavaScript 才能浏览图库。</noscript><script type="module" src="/theme.js"></script><script type="module" src="/gallery-app/gallery.js"></script></body></html>`, { headers: { 'Content-Type': 'text/html; charset=UTF-8', 'Cache-Control': 'public, max-age=60' } });
}

function gallerySlugsFromRequest(requestUrl, prefix) {
    const pathname = new URL(requestUrl).pathname;
    if (!pathname.startsWith(prefix)) return null;
    const parts = pathname.slice(prefix.length).split('/').filter(Boolean);
    if (parts.length !== 2) return null;
    try {
        return { ownerSlug: decodeURIComponent(parts[0]), albumSlug: decodeURIComponent(parts[1]) };
    } catch {
        return null;
    }
}

function escapeAttribute(value) {
    return String(value).replace(/[&<>"]/g, (char) => ({ '&': '&amp;', '<': '&lt;', '>': '&gt;', '"': '&quot;' }[char]));
}
