...
const topbar = `${brand}<div class="top-actions"><a href="/">发现</a><span class="top-label">分享图库</span>${themeButton}</div>`;
...
function itemMedia(item, cover = false) {
  ...
  if (isVideo(original)) return `<video src="${escapeHtml(original)}" muted loop playsinline preload="metadata" aria-label="${escapeHtml(label)}"></video>${cover ? '' : `<span class="type-label">${videoLabel(original)}</span>`}`;
...
async function init() {
  ...
  try {
    ...
    if (!response.ok) throw new Error(response.status === 404 ? '这个图库不存在，或分享链接无效。' : '图库暂时无法读取。');
    ...
    root.innerHTML = `<div class="gallery-shell">
      ...
      <main class="gallery-main">
        ...
        <p class="gallery-note">图库内容可通过此分享链接访问。</p>
      </main>
    </div>`;
